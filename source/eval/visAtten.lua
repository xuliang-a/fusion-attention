...
local path = '/home/zt/ai/fastbabytalk/source/'

local atten = torch.load(path .. 'atten_gt_test_1.t7')
local caption = read_json(path ..'visu_gt_test.json')

local json_file = read_json('/home/zt/ai/fastbabytalk/dataset/coco/cocotalk.json')
local id2path = {}
for i = 1,5000 do
...
local num = #caption
local cap_all = {}
local scale_map = torch.FloatTensor(num,16, 224, 224)
local atten_weight = torch.FloatTensor(num, 16)
local atten_map_original = torch.FloatTensor(num, 16, 7, 7)

f = io.open('cocovisu.txt', 'w')
for t = 1, num do
    local cap = caption[t]['caption']
    local img_id = caption[t]['image_id']
    local atten_map = atten[{{},{t},{}}]:contiguous():view(16,50)
