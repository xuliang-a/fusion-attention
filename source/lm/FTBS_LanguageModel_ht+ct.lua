...
local LSTM = require 'lm.FTBS_LSTM'
local attention = require 'lm.FTBS_attention_ht+ct'
...
function layer:getModulesList()
  return {self.core, self.lookup_table,self.attentions}
end
...
function layer:sample(imgs, opt)
  ...
  for t=1,self.seq_length+2 do
    ...
    if t > 1 then
      local atten_input = {h_out, p_out, imgs}
...
function layer:sample_beam(imgs, opt)
  ...
  for k=1,batch_size do
    ...
    local done_beams = {}
    -- local imgk = imgs:narrow(1,k,1):view(feat_dim)
    -- print("imgk1:",imgk)

    -- imgk:expand(beam_size, feat_dim) -- k'th image feature expanded out
    -- print("imgk2:",imgk)

    local imgk = imgs[{ {k,k} }]:expand(beam_size, feat_dim):contiguous() -- k'th image feature expanded out
    ...
    for t=1,self.seq_length+2 do
      ...
      if t > 1 then 
        local h_out = out[self.num_state+1]
        local p_out = out[self.num_state+2]
        -- imgk = torch.Tensor(2,512):zero():cuda()
        local atten_input = {h_out, p_out, imgk}
        logprobs = self.attention:forward(atten_input)
      end
    end
...
function layer:updateOutput(input)
  ...
  for t=1,self.seq_length+2 do
    ...
    if not can_skip then
      ...
      self.atten_inputs[t] = {h_out, p_out, imgs}
