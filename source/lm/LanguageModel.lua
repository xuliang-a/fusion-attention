...
local LSTM = require 'lm.LSTM'

local attention = require 'lm.attention'
local img_embedding = require 'lm.img_embedding'
...
function layer:sample_beam(inputs, opt)
  ...
  for k=1,batch_size do
    ...
    for t=1,self.seq_length+1 do
      ...
      local h_out = out[self.num_state+1]
      local p_out = out[self.num_state+2]
      local atten_input = {h_out, p_out, conv_feat_k, conv_feat_embed_k, prev_h}
      logprobs = self.attention:forward(atten_input)
...
function layer:updateOutput(input)
  ...
  for t = 1,self.seq_length+1 do
    ...
    if not can_skip then
      ...
      self.atten_inputs[t] = {h_out, p_out, self.conv_feat,  self.conv_feat_embed,prev_h}
      local atten_out = self.attentions[t]:forward(self.atten_inputs[t])

      -- wcy:: 提取output的第一个维度，从第t个位置开始放，每次放一个
      -- wcy:: 将每个时间步输出的（5，vocab_size+1）的概率信息放在outout里面
